...
def render_rays(nerf: nn.Module,
    ...
    N_rays = rays.shape[0] # rays 为batch_size行8列 每一列分别为：位置(3) 指向(3) nera(1) far(1) float32

    rays_o, rays_d = rays[:, 0:3], rays[:, 3:6]  # both (N_rays, 3)
    near, far = rays[:, 6:7], rays[:, 7:8]  # both (N_rays, 1)
    if image_indices is not None: # size([batch_size])
        image_indices = image_indices.unsqueeze(-1).unsqueeze(-1) # size([batch_size, 1, 1]) float32

    perturb = hparams.perturb if nerf.training else 0 # perturb=0
    last_delta = 1e10 * torch.ones(N_rays, 1, device=rays.device) # batch_size*1 float32
    if bg_nerf is not None: # 如果bg_nerf不为空，计算背景
        fg_far = _intersect_sphere(rays_o, rays_d, sphere_center, sphere_radius)
        fg_far = torch.maximum(fg_far, near.squeeze())
        rays_with_bg = torch.arange(N_rays, device=rays_o.device)[far.squeeze() > fg_far] # 从0~65536中选出需要计算背景的ray rays_with_bg的大小<=N_rays rays_with_bg[774]=1098
    ...
    else: # bg_nerf为空，只计算前景
        rays_o = rays_o.view(rays_o.shape[0], 1, rays_o.shape[1])
        rays_d = rays_d.view(rays_d.shape[0], 1, rays_d.shape[1])

    # Sample depth points
    z_steps = torch.linspace(0, 1, hparams.coarse_samples, device=rays.device)  # (N_samples) 0~1均匀分成coarse_smples份
    z_vals = near * (1 - z_steps) + far * z_steps # 加权
    ...
    # del results["depth_fine"] # 节省显存开销
    if bg_nerf is not None:
        types = ['fine' if hparams.fine_samples > 0 else 'coarse'] # fine
        if hparams.use_cascade and hparams.fine_samples > 0: # skip
            types.append('coarse')
    ...
    if bg_nerf is not None and not bg_nerf_rays_present and 'RANK' in os.environ and nerf.training: # skip
...
def _inference(results: Dict[str, torch.Tensor],
    ...
    if flip and 'zvals_coarse' not in results:
        xyz = torch.flip(xyz, dims=[-2, ]) # (17, 128, 7)
        z_vals = torch.flip(z_vals, dims=[-1, ])

    xyz_ = xyz.view(-1, xyz.shape[-1]) # (17*128, 7)
    ...
    if hparams.pos_dir_dim == 0: # 否，跳过
        ...
        for i in range(0, B, hparams.model_chunk_size):
            ...
            if hparams.use_cascade: # False
                model_chunk = nerf(typ == 'coarse', xyz_chunk, sigma_noise=sigma_noise)
            else:
                model_chunk = nerf(xyz_chunk, sigma_noise=sigma_noise) # xyz_chunk为11维
