...
def get_ray_directions(W: int, H: int, fx: float, fy: float, cx: float, cy: float, center_pixels: bool,
    ...
    directions = \
        torch.stack([(i - cx) / fx, -(j - cy) / fy, -torch.ones_like(i)], -1)  # (H, W, 3) float32 此时的directions[i,j]表示图像上第i行(H方向) 第j列(W方向) 像素点对应的相机系下坐标 directions[0,0]=[-0.7721, 0.5789, -1] directions[1,0]=[-0.7721, 0.5776, -1] 返回的向量中对应的为(x, y, z)
